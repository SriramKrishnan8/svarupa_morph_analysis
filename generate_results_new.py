...
script, inp, sh_res_sa, sh_res_hy, scl_res_sa, scl_res_hy, dcs_rv_res_sa, dcs_rv_res_hy, dcs_av_res_sa, dcs_av_res_hy, final_res, final_res_all, cache, all_res = sys.argv
...
for i in tqdm(range(len(sh_dict_lst_sa))):
    sh_item_sa, sh_sa = get_confirmation(sh_dict_lst_sa[i])
    sh_item_hy, sh_hy = get_confirmation(sh_dict_lst_hy[i])
    ...
    if sh_item_sa["input"] in cache_dict:
        final_dict_lst_all.append(cache_dict[sh_item_sa["input"]])
    elif success:
        new_dict = {}
