...
script, inp, res_sa, res_hy, final_res, final_res_all, cache, all_res, source = sys.argv
...
dict_lst_sa = get_contents(res_sa)
dict_lst_hy = get_contents(res_hy)
...
print("Generating " + source + " Results...")
for i in tqdm(range(len(dict_lst_sa))):
    line = inp_list[i + 1]
    word = line.split("\t")[0]
    
    item_sa, st_sa = get_confirmation(dict_lst_sa[i])
    item_hy, st_hy = get_confirmation(dict_lst_hy[i])

    # scl_item_sa, scl_sa = get_confirmation(scl_dict_lst_sa[i])
    # scl_item_hy, scl_hy = get_confirmation(scl_dict_lst_hy[i])
    ...
    success = True
    if st_sa:
        final_dict_lst.append(item_sa)
    elif st_hy:
        final_dict_lst.append(item_hy)
    # elif scl_sa:
    ...
    if item_sa["input"] in cache_dict:
        ...
        new_dict = merge_sa_hy(item_sa, item_hy, item_sa)
        ...
        cache_dict[item_sa["input"]] = new_dict
